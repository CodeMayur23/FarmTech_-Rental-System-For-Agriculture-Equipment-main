$(document).ready(function(){
	...
	*/
	...
	$(document).ready(function () {
		...
		getCartItem(); // Call on page load
	
		// ✅ Update Quantity & Total Price Dynamically
		$("body").on("keyup change", ".qty", function (event) {
			event.preventDefault();
			...
			// ✅ Recalculate Net Total, Deposit Amount, and Total Cost
			var net_total = 0;
			var deposit_amount = 0;
			var total_cost = 0;
		
			$(".total").each(function () {
				var item_total = parseFloat($(this).val()) || 0;
				net_total += item_total;
				deposit_amount += item_total;
				total_cost += 2 * item_total;
			});
		
			$(".net_total").html("Total: Rs " + net_total);
			$(".deposit_amount").html("Deposit Amount: <strong>Rs " + deposit_amount + "</strong>");
			$(".total_cost").html("<strong>Total Cost (including deposit): Rs " + total_cost + "</strong>");
			...
			$.ajax({
				...
				success: function (response) {
					$(".net_total").html("<b>Total: Rs " + response.final + "</b>");
					$(".deposit_amount").html("Deposit Amount: <strong>Rs " + response.deposit_amount + "</strong>");
					$(".total_cost").html("<strong>Total Cost (including deposit): Rs " + response.total_cost + "</strong>");
				},
				error: function () {
		...
		$("body").on("click", ".remove", function () {
			...
			$.ajax({
				...
				success: function (data) {
					$("#cart_msg").html(data);
					row.remove(); // Remove item from UI
					count_item();
					checkOutDetails();
				}
			});
...
$(document).ready(function () {
    $(document).on("click", ".showDescription", function () {
        var productDesc = $(this).closest(".panel-footer").find(".product-description");
        var productVideo = $(this).closest(".panel-footer").find(".product-video");

        if (productDesc.is(":visible")) {
            productDesc.slideUp();
            productVideo.slideUp();
        } else {
            $(".product-description").slideUp();
            $(".product-video").slideUp();
            productDesc.slideDown();
            productVideo.slideDown();
        }
    });
});
